...
export class HistoryAppointmentsComponent implements OnInit {
      ...
      .subscribe({
        next: (allAppointments: any[]) => {
          ...
          allAppointments.forEach(apt => {
            console.log(`Cita ID: ${apt.id}, Patient_ID: ${apt.patient_id} (tipo: ${typeof apt.patient_id})`);
          });
          ...
          const patientAppointments = allAppointments.filter(apt => {
            ...
            console.log(`Comparando: ${aptPatientId} === ${currentPatientId} = ${aptPatientId === currentPatientId}`);
            
            return aptPatientId === currentPatientId;
          });
          
          console.log('Citas del paciente después del filtro:', patientAppointments);
          
          // Si aún no hay citas, intentar con comparación más flexible
          if (patientAppointments.length === 0) {
            console.log('⚠️ No se encontraron citas con filtro estricto, probando filtro flexible...');
            ...
            if (userFromStorage) {
              ...
              const appointmentsByName = allAppointments.filter(apt => 
                apt.patient_name && apt.patient_name.toLowerCase().includes(patientName.toLowerCase())
              );
  ...
  processAppointments(patientAppointments: any[]) {
    ...
    this.patientHistory.sort((a, b) => new Date(b.date + ' ' + b.time).getTime() - new Date(a.date + ' ' + a.time).getTime());
  ...
  calculateStats() {
    ...
    this.completedAppointments = this.patientHistory.filter(apt => apt.status === 'completed').length;
    this.cancelledAppointments = this.patientHistory.filter(apt => apt.status === 'cancelled').length;
    this.noShowAppointments = this.patientHistory.filter(apt => apt.status === 'no_show').length;
  }


  extractFilterOptions() {
    // Años disponibles
    const years = [...new Set(this.patientHistory.map(apt => new Date(apt.date).getFullYear().toString()))];
    this.availableYears = years.sort((a, b) => parseInt(b) - parseInt(a));
    
    // Especialidades disponibles
    const specialties = [...new Set(this.patientHistory.map(apt => apt.physician_specialty))];
    this.availableSpecialties = specialties.filter(spec => spec && spec !== 'Especialidad no especificada').sort();
  }
  ...
  checkScrollIndicator() {
    ...
    if (appointmentsContent) {
      const hasScroll = appointmentsContent.scrollHeight > appointmentsContent.clientHeight;
      const isAtTop = appointmentsContent.scrollTop < 50;
      this.showScrollIndicator = hasScroll && isAtTop && this.filteredHistory.length > 3;
    }
  }
  ...
  onScroll(event: any) {
    ...
    this.showScrollIndicator = atTop && this.filteredHistory.length > 3 && 
  ...
  applyFilters() {
    this.filteredHistory = this.patientHistory.filter(apt => {
      ...
      const matchesYear = !this.selectedYear || new Date(apt.date).getFullYear().toString() === this.selectedYear;
      const matchesSpecialty = !this.selectedSpecialty || apt.physician_specialty === this.selectedSpecialty;
      const matchesDoctor = !this.searchDoctor || apt.physician.toLowerCase().includes(this.searchDoctor.toLowerCase());
